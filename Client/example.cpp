...
static void InitSteamDatagramConnectionSockets( )
{
	...
	SteamDatagramErrMsg errMsg;
	if ( !SteamDatagramClient_Init( errMsg ) ) FatalError( "SteamDatagramClient_Init failed.  %s", errMsg );
...
public:
	void Run( uint16 nPort )
	{
		...
		if ( m_hListenSock == k_HSteamListenSocket_Invalid ) FatalError( "Failed to listen on port %d"
				, nPort
			);
		m_hPollGroup = m_pInterface->CreatePollGroup( );
		if ( m_hPollGroup == k_HSteamNetPollGroup_Invalid ) FatalError( "Failed to listen on port %d"
				, nPort
			);
...
private:
	...
	{
		m_pInterface->SendMessageToConnection( conn
							, str
							, ( uint32 ) strlen( str )
			...
			case k_ESteamNetworkingConnectionState_Connecting :
				{
					...
					if ( !m_pInterface->
						SetConnectionPollGroup( pInfo->m_hConn, m_hPollGroup ) ) {
						m_pInterface->CloseConnection( pInfo->m_hConn, 0, nullptr, false );
					...
					if ( m_mapClients.empty( ) ) SendStringToClient( pInfo->m_hConn
							, "Thou art utterly alone."
						);
					else {
						sprintf( temp
							, "%d companions greet you:"
							, ( int ) m_mapClients.size( )
							);
					...
					sprintf( temp
						, "Hark!  A stranger hath joined this merry host.  For now we shall call them '%s'"
						, nick
						);
...
private:
	...
	void OnSteamNetConnectionStatusChanged( SteamNetConnectionStatusChangedCallback_t *pInfo )
	{
		...
		switch ( pInfo->m_info.m_eState ) {
			...
			case k_ESteamNetworkingConnectionState_Connected : Printf( "Connected to server OK" );
				break;
...
int main( int argc, const char *argv[ ] )
{
	...
	for ( int i = 1 ; i < argc ; ++i ) {
		...
		if ( !strcmp( argv[ i ], "--port" ) ) {
			...
			nPort = atoi( argv[ i ] );
			if ( nPort <= 0 || nPort > 65535 ) FatalError( "Invalid port %d", nPort );
			continue;
		}

		// Anything else, must be server address to connect to
		if ( bClient && addrServer.IsIPv6AllZeros( ) ) {
			if ( !addrServer.ParseString( argv[ i ] ) ) FatalError( "Invalid server address '%s'"
					, argv[ i ]
				);
			if ( addrServer.m_port == 0 ) addrServer.m_port = DEFAULT_SERVER_PORT;
			continue;
		}
	...
	if ( bClient == bServer || ( bClient && addrServer.IsIPv6AllZeros( ) ) ) PrintUsageAndExit( );
